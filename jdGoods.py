...
def main(keyword):
    ...
    for i in range(3, page * 2, 2):
        ...
        if j == 1:
            url = 'https://search.jd.com/Search?keyword={}&enc=utf-8&page={}&s={}&click=0'.format(keyword, i, j)
        else:
            url = 'https://search.jd.com/Search?keyword={}&enc=utf-8&page={}&s={}&click=0'.format(keyword, i,
                                                                                                  (j - 1) * 50)
        driver.get(url)
        time.sleep(1)
        ...
        print(f'正在爬取第{j}页')
        j += 1
    wb.save('D:\Document\京东{}相关商品信息.xlsx'.format(keyword))
